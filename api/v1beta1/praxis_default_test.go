...
// (controllers.resolvePraxisMode is the same predicate). Only an explicit true opts in; an absent
// spec.praxisMode — what a greenfield CR and an upgraded CR both carry — reads as legacy, which is
// what keeps OGX serving /v1/responses in 3.6.
func TestIsPraxisModeEnabled_DefaultsToDisabled(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "spec.praxisMode present but enabled unset: disabled at the Go level",
			praxisMode: &PraxisModeSpec{},
			want:       false,
		},
		{
		...
		{
			name: "praxisSelector without enabled does not opt in on its own",
			praxisMode: &PraxisModeSpec{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := isPraxisModeEnabled(server); got != tt.want {
